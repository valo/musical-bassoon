...
import {LiquidateModule, ILiquidateModule} from "v2-matching/src/modules/LiquidateModule.sol";
...
contract MatchingHelpers is Test {
    ...
    function _deployMatching(ISubAccounts subAccounts, address cash, DutchAuction auction, uint256 feeRecipient)
        internal
    {
    ...
    function _signAction(bytes32 actionHash, uint256 signerPk) internal view returns (bytes memory) {
        //    console2.log("typed hash");
        //    console2.logBytes32(domainSeparator);
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(signerPk, MessageHashUtils.toTypedDataHash(domainSeparator, actionHash));
        return bytes.concat(r, s, bytes1(v));
    }

    function _encodeDepositData(uint256 amount, address asset, address newManager)
        internal
        pure
    ...
    function _encodeWithdrawData(uint256 amount, address asset) internal pure returns (bytes memory) {
        IWithdrawalModule.WithdrawalData memory data =
            IWithdrawalModule.WithdrawalData({asset: asset, assetAmount: amount});
    ...
    function _encodeLiquidateData(
        uint256 liqAccId,
        uint256 cashTransfer,
        uint256 percent,
        int256 priceLimit,
        uint256 lastSeenTradeId,
        bool merge
...
contract MatchingBase is PMRMTestBase, MatchingHelpers {
    ...
    // signer
    uint256 internal camAcc;
    uint256 internal camPk;
    address internal cam;

    uint256 internal dougAcc;
    uint256 internal dougPk;
    address internal doug;

    uint256 referenceTime;
    uint256 defaultCallId;

    uint256 cashDeposit = 10000e18;
    ...
    function _createNewAccount(address owner) internal returns (uint256) {
        // create a new account
        uint256 newAccountId = subAccounts.createAccount(owner, IManager(address(pmrm)));
        vm.startPrank(owner);
    ...
    function _openCLOBAccount(address owner, uint256 accountId) internal {
        vm.startPrank(owner);
